      ...
      <Slider class="carousel carousel-center w-full col-span-full row-span-full gap-2">
        {images?.map((image, index) => (
          <Slider.Item index={index} class="carousel-item w-full flex items-center">
            <Image class="w-full h-max" src={image} width={550} />
          </Slider.Item>
        ))}
      </Slider>
          ...
          <li class="carousel-item">
            <Slider.Dot index={index}>
              <div class="w-14 h-12 py-5 border-[1px] border-black flex justify-center items-center opacity-40 group-disabled:opacity-100">
                <Image class="w-[40px] h-max" src={image} width={80} />
              </div>
            </Slider.Dot>
          </li>
        ))}
  ...
  return (
    <>
      <div class="flex items-center justify-center z-10 col-start-1 row-start-2">
        <Slider.PrevButton>
          <Icon class="text-white" size={26} id="ChevronLeft" strokeWidth={5} />
        </Slider.PrevButton>
      </div>
      <div class="flex items-center justify-center z-10 col-start-3 row-start-2">
        <Slider.NextButton>
          <Icon class="text-white" size={26} id="ChevronRight" strokeWidth={5} />
        </Slider.NextButton>
      </div>
    </>
  );
